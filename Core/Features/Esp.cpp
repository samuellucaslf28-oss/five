...
bool IsVisible = true;
if (Cfg->VisibleCheck && !IsLocalPlayer)
{
    IsVisible = Core::Features::Visibility::IsVisible(Ped);
}
...
D3DXVECTOR2 EntityTop = Core::SDK::Game::WorldToScreen( HeadPos + D3DXVECTOR3( 0, 0, 0.2 ) );
D3DXVECTOR2 EntityBottom = Core::SDK::Game::WorldToScreen(
Ped->GetBonePosDefault( 8 ) - ( g_Offsets.CurrentBuild >= 2802 ? D3DXVECTOR3( 0, 0, 2 ) : D3DXVECTOR3( 0, 0, 1.2 ) )
);
...
if ( Cfg->Box )
{
    ImColor CurrentBoxCol = Cfg->BoxCol;
    if (Cfg->VisibleCheck && !IsVisible) {
        CurrentBoxCol = ImColor(255, 0, 0, (int)(Cfg->BoxCol.Value.w * 255));
    }
...
DrawList->AddRect( { BoxLeft + 1, EntityTop.y + 1 }, { BoxRight - 1, EntityBottom.y - 1 }, ImColor( 0, 0, 0, 200 ), 0, 0, 1.5 );
DrawList->AddRect( { BoxLeft, EntityTop.y }, { BoxRight, EntityBottom.y }, CurrentBoxCol, 0, 0, 1.5 );
}
if (Cfg->Skeleton)
{
...
ImColor SkelColor = Cfg->SkeletonCol;
if (Health <= 0) {
    SkelColor = ImColor(255, 0, 0);
}
else if (Cfg->VisibleCheck && !IsVisible) {
    SkelColor = ImColor(255, 0, 0, (int)(Cfg->SkeletonCol.Value.w * 255));
}
DrawList->AddLine(ImVec2(ScreenHead.x, ScreenHead.y), ImVec2(Neck.x, Neck.y), SkelColor, stick);
DrawList->AddLine(ImVec2(LeftUperarm.x, LeftUperarm.y), ImVec2(RightUperarm.x, RightUperarm.y), SkelColor, stick);
DrawList->AddLine(ImVec2(RightUperarm.x, RightUperarm.y), ImVec2(RightFormArm.x, RightFormArm.y), SkelColor, stick);
DrawList->AddLine(ImVec2(LeftUperarm.x, LeftUperarm.y), ImVec2(LeftFormArm.x, LeftFormArm.y), SkelColor, stick);
DrawList->AddLine(ImVec2(RightFormArm.x, RightFormArm.y), ImVec2(RightHand.x, RightHand.y), SkelColor, stick);
DrawList->AddLine(ImVec2(LeftFormArm.x, LeftFormArm.y), ImVec2(LeftHand.x, LeftHand.y), SkelColor, stick);
DrawList->AddLine(ImVec2(Neck.x, Neck.y), ImVec2(Pelvis.x, Pelvis.y), SkelColor, stick);
DrawList->AddLine(ImVec2(Pelvis.x, Pelvis.y), ImVec2(LeftThigh.x, LeftThigh.y), SkelColor, stick);
DrawList->AddLine(ImVec2(Pelvis.x, Pelvis.y), ImVec2(RightThigh.x, RightThigh.y), SkelColor, stick);
DrawList->AddLine(ImVec2(LeftThigh.x, LeftThigh.y), ImVec2(LeftCalf.x, LeftCalf.y), SkelColor, stick);
DrawList->AddLine(ImVec2(RightThigh.x, RightThigh.y), ImVec2(RightCalf.x, RightCalf.y), SkelColor, stick);
DrawList->AddLine(ImVec2(LeftCalf.x, LeftCalf.y), ImVec2(Lfoot.x, Lfoot.y), SkelColor, stick);
DrawList->AddLine(ImVec2(RightCalf.x, RightCalf.y), ImVec2(Rfoot.x, Rfoot.y), SkelColor, stick);
}
if ( Cfg->HeadCircle )
{
...
ImColor CircleColor = Cfg->SkeletonCol;
if (Health <= 0) {
CircleColor = ImColor(255, 0, 0);
}
ImVec2 center( head_screen_pos.x, head_screen_pos.y );
DrawList->AddCircle( center, CircleValue, CircleColor, 999, 1.5 );
}
}
}
